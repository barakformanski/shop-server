...
import { Button, Upload, Select, Table, Input, InputNumber, Form } from "antd";
...
const AdminPage = (props) => {
  ...
  const uploadImage = () => {
    ...
    axios
      .post(`${PREFIX}/uploadNewProductImage`, { base64 })
  ...
  function DeleteProduct() {
    axios
      ...
      .then((res) => {
        alert(
          `המוצר ${productTitle}, שמספרו המזהה ${selectedProductToDelete} נמחק`
        );
      });
  }
    ...
    <div className="adminPage" dir="rtl">
      <div className="delete_product">
        ...
        <Form onFinish={onFinish}>
          ...
          >
            <Select placeholder="בחר מוצר למחיקה" onChange={onChange}>
          ...
          <button className="send id" onClick={DeleteProduct}>
            לחץ כדי למחוק
          </button>
          <input
            className="deletedProductInput"
            type="text"
            size="big"
            ref={idTodelete}
            placeholder="  או הכנס את ה id של המוצר"
          />
        </Form>
        ...
        <div className="update_quantity">
          ...
          <Select placeholder="בחר מוצר לעדכון" onChange={onChange}>
            {products.map((product) => (
              <Option
                key={product._id}
                value={product._id}
          ...
          <input
            ...
            type="text"
            ref={idToUpdate}
            placeholder="של המוצר שברצונך לעדכן id הכנס את ה"
          />
          <button className="send id" onClick={idToUpdate}>
            לחץ כדי לעדכן
          </button>
        </div>
